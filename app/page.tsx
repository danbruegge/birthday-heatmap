...
function parseVCARD(data: string): Person[] {
  return data.split("BEGIN:VCARD").slice(1).map((vcard) => {
...
export default function Home() {
  ...
  const monthWeightList = MONTHS.map((_, month) => daysOfMonth[month].reduce((acc, curr) => acc + curr, 0))
      ...
      <div className="flex flex-col gap-12">
        ...
        <div className="flex gap-8 sm:gap-4 flex-wrap">
          {MONTHS.map((monthData, month) => {
            let monthWeight = Math.min(Math.round(monthWeightList[month] / maxMonthWeight * 100) - 20);
            monthWeight = Math.round(monthWeight / 10) * 10;

            return (
              <div key={monthData.name} className="w-auto sm:w-48 flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 rounded" style={{
                    backgroundColor: `hsl(73, 60%, ${monthWeight}%)`,
                    border: `1px solid hsl(73, 10%, 20%)`,
                  }}></div>
                  <h2 className="w-24 pr-2 opacity-80 text-xl">{monthData.name}</h2>
                </div>
                <div className="flex flex-wrap gap-1">
                  {daysOfMonth[month].map((dayWeight, day) => {
                    ...
                    return (
                      <div key={day} className={`w-6 h-6 flex items-center justify-center rounded`} style={{
                        backgroundColor: dayWeight === 0 ? "hsl(73, 15%, 10%)" : `hsl(73, 60%, ${lightValue}%)`,
                        border: `1px solid hsl(73, 10%, 20%)`
                      }}></div>
                    )
                  })}
